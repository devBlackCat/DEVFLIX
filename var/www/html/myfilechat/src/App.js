...
const App = () => {
  ...
  useEffect(() => {
    const fetchMessages = async () => {
      try {
        const response = await axios.get('https://ad0ad0.synology.me:21511/chat/list.php');
  ...
  const fetchOlderMessages = async () => {
    ...
    try {
        ...
        const response = await axios.post(
            'https://ad0ad0.synology.me:21511/chat/history.php',
...
const sendMessage = async () => {
    if (message.trim()) {
        try {
            ...
            // 메시지 객체를 서버로 전송합니다.
            const response = await axios.post('https://ad0ad0.synology.me:21511/chat/message.php', JSON.stringify(messageData), {
                headers: {
...
const uploadImage = async (formData) => {
  try {
    const response = await axios.post('https://ad0ad0.synology.me:21511/chat/image.php', formData, {
      headers: {
...
const uploadFile = async (formData) => {
  try {
    const response = await axios.post('https://ad0ad0.synology.me:21511/chat/file.php', formData, {
      headers: {
        'Content-Type': 'multipart/form-data',
      },
    });
  ...
  <div className="container">
    ...
    {showAttachments && (
      <div className="attachmentsContainer">
        <input type="file" accept="*/*" id="file-upload" onChange={pickFile} className="fileInput" style={{ display: 'none' }} />
        <label htmlFor="file-upload" className="uploadLabel">파일 업로드</label>
        <input type="file" accept="image/*" id="image-upload" onChange={pickImage} className="fileInput" style={{ display: 'none' }} />
        <label htmlFor="image-upload" className="uploadLabel">사진 업로드</label>
      </div>
    )}
